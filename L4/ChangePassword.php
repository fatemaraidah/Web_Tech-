...
if(empty($p) || empty($np) || empty($cp)){
		...
		$npErr =" Must contain minimum eight characters, at least one letter, one number and one special character!";
	     }
	     else{  
		    $cu = "jb";
	    ...
	    if(!strcmp($p, $np)){
          $npErr = "New Password must not be same as the Current Password!";
	     }
	     else{
	    if(strcmp($np, $cp))
	     {
